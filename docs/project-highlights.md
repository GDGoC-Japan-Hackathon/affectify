...
本プロジェクトが目指したのは、「設計品質」「全体把握」「継続的改善」の 3 つを同時に実現するプラットフォームです。

特に、次のような問題を解決することを狙っています。

- AI が大量のコードを生成することで、開発者の認知負荷が上がり、全体像を把握しにくくなる問題
- コードベースが大規模になるほど、設計そのものの難易度が上がる問題
- 従来のエディタの「上から下に書く」制約が、頭の中のイメージと実際のコード配置の間にギャップを生む問題
- 大規模コードで目的の関数やそのつながりを把握するのに時間がかかる問題
- ドキュメントや図と実装コードが乖離し、設計思想が失われやすい問題
- プロジェクト途中参加者が、ファイル構造だけでは「なぜこの実装になったか」を理解しにくい問題
...
WhiteCoder は、従来の IDE をそのまま置き換えるというより、「思考を整理し、ロジックの種を育てる」ための Pre-IDE に近い発想で設計しています。

中心にある考え方は次の通りです。

- ホワイトボードのような自由度の高い空間でコードを扱えるようにすることで、開発者の頭の中のイメージをそのまま配置に落とし込めるようにする
- AI が作った複雑なコードも、グラフとして可視化することで問題点を視覚的に捉えやすくする
- 「近くにあるものは関係が深い」という感覚を使って、空間的近接性そのものをロジック整理の手段にする
- 設計書とコードを別世界のものにせず、同じ workspace の中で行き来できるようにする

この発想により、「設計図を描いてから別のエディタに書き写す」という二度手間を減らし、設計と実装の間のギャップを小さくすることを目指しています。

## 3. 実現のための具体的な機能

上記の問題意識と発想を具体化するために、次の機能を実装しています。

- 関数ごとにノード化し、呼び出し関係や実装関係をエッジとしてホワイトボード上に可視化する機能
- AI が設計に対して評価を付け、問題点を指摘するレビュー機能
- 自動並び替え機能
  - 呼び出し関係を考慮した整列
  - ファイルごとの整列
- 設計書を作成し、その設計ルールに対して AI がレビューする機能
- メモ、手書き、範囲選択、画像挿入など、ホワイトボードらしい補助機能
- 選択ノードとその周辺だけを強調し、それ以外を薄くするフォーカス機能
- 設計書やルールを共有しながら開発できる機能

加えて、日常的な使いやすさを高めるために、
...
- テーマ性
  現代的な課題である「設計の困難化」「全体把握の難しさ」「AI 生成コード時代の認知負荷」に対して、ホワイトボード型コードエディタという形で解決策を提示しています。
- 堅牢性
  複雑な関係性をグラフとして可視化し、さらに review / apply / graph rebuild のループを持つことで、単なる見た目の可視化に終わらない構造的な改善が可能です。
- Google 技術の活用
  Vertex AI を用いて、レビュー生成、チャット、決定内容の下書き、実際の反映までを一貫して支えています。
- 異端性
  コードを「上から下に書くテキスト」だけでなく、「空間に配置して考える対象」として扱っている点が特徴です。人間の設計負荷や認知負荷を減らす新しい入口として意味があります。
