...
func main() {
	...
	analysisService := service.NewAnalysisService(db, userRepository)
	designGuideService := service.NewDesignGuideService(db, userRepository)
	nodeService := service.NewNodeService(db, userRepository)
	...
	analysisHandler := handler.NewAnalysisServiceHandler(analysisService)
	designGuideHandler := handler.NewDesignGuideServiceHandler(designGuideService)
	nodeHandler := handler.NewNodeServiceHandler(nodeService)
	...
	analysisPath, analysisHTTPHandler := apiv1connect.NewAnalysisServiceHandler(analysisHandler, interceptors)
	mux.Handle(analysisPath, analysisHTTPHandler)
	...
	nodePath, nodeHTTPHandler := apiv1connect.NewNodeServiceHandler(nodeHandler, interceptors)
	mux.Handle(nodePath, nodeHTTPHandler)
